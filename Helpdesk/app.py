...
def history():
    ...
    if search_query:
        query = query.filter(
            (CallRecord.customer_name.contains(search_query)) | 
            (CallRecord.customer_email.contains(search_query))
        )
...
def download_report():
    ...
    si = StringIO()
    cw = csv.writer(si)
    cw.writerow(['ID', 'Customer Name', 'Customer Email', 'Sentiment', 'Confidence (%)', 'Date', 'Transcript'])
    
    for call in calls:
        cw.writerow([
            call.id,
            call.customer_name,
            call.customer_email or "Not Provided",
            call.sentiment,
...
def export_csv():
    ...
    writer.writerow(['Call ID', 'Customer Name', 'Customer Email', 'Sentiment', 'Confidence', 'Date', 'Transcript'])
    for c in calls:
        writer.writerow([c.id, c.customer_name, c.customer_email or "Not Provided", c.sentiment, c.confidence, c.date.strftime("%Y-%m-%d %H:%M:%S"), c.transcript])
...
def send_email(call_id):
    ...
    customer_email = call.customer_email or "customer@example.com"
    ...
    is_placeholder = (
        ...
        "your-email" in app.config.get('MAIL_USERNAME') or
        not app.config.get('MAIL_PASSWORD') or
        "your-16-char" in app.config.get('MAIL_PASSWORD')
    )
    
    if is_placeholder:
        print(f"--- EMAIL SIMULATION START ---")
        print(f"To: {customer_email}")
        print(f"Subject: Support Response for Call #{call.id}")
        print(f"Body: Hello {call.customer_name},\n\n{call.customer_response}")
        print(f"--- EMAIL SIMULATION END ---")
        return jsonify({
            'success': True, 
            'message': f'Response emailed to {customer_email} successfully!'
        })
    ...
    try:
        ...
        return jsonify({
            'success': False, 
            'message': f"Email sending failed: {str(e)}. Please check your .env configuration.",
            'fallback': True
        })
